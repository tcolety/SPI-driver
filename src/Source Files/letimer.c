 ...
 * @date 9/12/2021
 * @brief Functions controlling the LETIMER peripheral
 *
 */
...
void letimer_pwm_open(LETIMER_TypeDef *letimer, APP_LETIMER_PWM_TypeDef *app_letimer_struct){
	...
	unsigned int period_cnt = app_letimer_struct->period * LETIMER_HZ; //LETIMER_HZ
	...
	//Turns on clock by putting true to AND gate
	if (letimer == LETIMER0){
  ...
  // ZZZ are values that you must specify for this PWM specific driver from the online HAL documentation
	...
	while(letimer->SYNCBUSY);                                                         //COMMENTED OUT
  ...
  /* Enabling interrupts*/
	...
	letimer->IEN = (LETIMER_IEN_COMP0*app_letimer_struct->comp0_irq_enable)
	    ...
	    | (LETIMER_IEN_UF*app_letimer_struct->uf_irq_enable);
...
/***************************************************************************//**
 * @brief
 *          Function checks if interrupt flags have been set, then adds
 *          corresponding event to scheduler
 * @details
 *          LETIMER0_IRQHandler() sets int_flag to equal the interrupts that
 *          have been flagged and are enabled, then clears the flagged interrupts.
 *          Next, the flagged event gets added to the scheduler
 * @note
 *          Each bit in int_flag corresponds to its own interrupt, uf, comp0,
 *          and comp1.
 ******************************************************************************/
void LETIMER0_IRQHandler(void){
  ...
  if(int_flag & LETIMER_IF_COMP0){
      ...
      add_scheduled_event(scheduled_comp0_cb); }   //process interrupt
  if(int_flag & LETIMER_IF_COMP1){
      EFM_ASSERT(!(LETIMER0->IF & LETIMER_IF_COMP1));
      add_scheduled_event(scheduled_comp1_cb); }   //process interrupt
  if(int_flag & LETIMER_IF_UF){
      EFM_ASSERT(!(LETIMER0->IF & LETIMER_IF_UF));
      add_scheduled_event(scheduled_uf_cb); }      //process interrupt
}
 ...
 *   This function blocks EM4 if turning on LETIMER and unblocks sleep mode
 *   when turning off LETIMER.
 *
 * @note
